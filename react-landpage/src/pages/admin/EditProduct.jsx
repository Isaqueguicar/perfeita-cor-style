...
const EditProduct = () => {
        ...
        try {
          ...
          if (productDetails) {
            setProduct({
              ...
              categoriaId: productDetails.categoriaId 
            });

            const apiCust = (productDetails.customizacoes || []).map(cust => {
              const estoqueMap = (cust.estoque || []).reduce((acc, stockItem) => {
                if(stockItem.tamanho) {
                   acc[stockItem.tamanho] = stockItem.quantidade;
                }
                return acc;
              }, {});
              ...
              return {
                id: cust.id, 
                tamanhos: Object.keys(estoqueMap).filter(key => TAMANHOS_OPCOES.includes(key)), 
                numeros: [],
                cores: cust.cores || [], 
                estoqueTamanhos: estoqueMap, 
                estoqueNumeros: {}, 
                imagem: null,
                preview: getImageUrl(cust.imagem), 
                suggestedColors: [], 
                isAnalyzing: false
              };
            });
            setCustomizations(apiCust);
          } else {
            setError('Produto não encontrado.');
          }
        } catch (err) {
          setError(err.message || 'Erro ao buscar dados do produto.');
        } finally {
          setIsFetching(false);
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    for (const cust of customizations) {
      ...
      if (!cust.id && !cust.imagem) {
        setError('Uma *nova* variação deve ter uma imagem.');
        return;
      }
    ...
    customizations.forEach((cust, index) => {
      ...
      if (cust.id) {
        formData.append(`produtoCustoms[${index}].id`, cust.id);
      }
      
      if (cust.imagem instanceof File) {
  ...
  return (
    <div className="create-product-page">
      ...
      <form onSubmit={handleSubmit} className="product-form">
        ...
        <div className="form-section">
          <h2>Variações</h2>
          <button type="button" onClick={addCustomization} className="add-button">Adicionar Nova Variação</button>
          {customizations.map((cust, index) => (
            <div key={cust.id || `new-${index}`} className="customization-card">
              <div className="card-header">
                <h3>{cust.id ? `Variação #${cust.id}` : `Nova Variação #${index + 1}`}</h3>
